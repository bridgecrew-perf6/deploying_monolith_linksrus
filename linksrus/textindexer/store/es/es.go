...
import (
	...
	"deploying_monolith_linksrus/linksrus/textindexer/index"
...
func (i *ElasticSearchIndexer) FindByID(linkID uuid.UUID) (*index.Document, error) {
	...
	//	At this point, I would like to point out that I only opted to use an inline, type-less approach to define the search query for simplicity.
	//	Ideally, instead of using maps, you would define nested structs for each portion of the query.
	//	Besides the obvious benefits of working with typed values, one other important benefit of
	//	working with structs is that we can switch to a much more efficient JSON encoder implementation
	//	that doesn't require the use of reflection. One such example is easyjson [10],
	//	which utilizes code generation to create efficient JSON encoder/decoders and promises a 4x-5x
	//	increase in speed over the JSON encoder implementation that ships with the Go standard library.
