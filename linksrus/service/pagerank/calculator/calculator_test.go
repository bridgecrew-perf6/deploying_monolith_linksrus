...
import (
	"context"
	"deploying_monolith_linksrus/linksrus/service/pagerank/calculator"
	"math"
	"math/rand"
	"strconv"
	"testing"
...
func (s *CalculatorTestSuite) assertConvergence(c *gc.C, numLinks, maxOutLinks int) {
	calc, err := calculator.NewCalculator(calculator.Config{ComputeWorkers: 32, MinSADForConvergence: 0.001})
...
func (s *CalculatorTestSuite) assertPageRankScores(c *gc.C, spec spec) {
	...
	calc, err := calculator.NewCalculator(calculator.Config{
		ComputeWorkers: 2,
		DampingFactor:  0.85,
	})
	c.Assert(err, gc.IsNil)
